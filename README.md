...
An **autoresearch loop** that made [FSRS-rs](https://github.com/open-spaced-repetition/fsrs-rs) parameter optimization **almost 100x faster** (its portable core is being upstreamed into FSRS-rs itself — [PR #411](https://github.com/open-spaced-repetition/fsrs-rs/pull/411) merged, [#417](https://github.com/open-spaced-repetition/fsrs-rs/pull/417) open) — so real Anki users spend less time staring at the optimizer's progress bar and more time doing reviews — **while only making it mildly less accurate**. An AI agent (Claude) proposes a change, measures it under a strict protocol, and keeps it only if it clears both the speed and the correctness bars. Inspired by AlphaEvolve and [Andrej Karpathy's "autoresearch" repo](https://github.com/karpathy/autoresearch). Also check out [my other autoresearch repo](https://github.com/Expertium/fsrs-autoresearch).
