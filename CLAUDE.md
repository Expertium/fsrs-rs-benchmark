...
**The slow era (as of ~iter23, ~×100 product / ~×96 anchored).** The big and medium levers are spent — autodiff removal, the O(N) expanding window, SIMD across cards, the hand-rolled optimizer, and (after iter23's windowed-only trade) BOTH minimax accuracy bands are now ~exhausted. From here, **accepted speedups will be rare**: most candidates will reject on the 5% median floor or a correctness band. **That is expected, not failure — do NOT get discouraged if your success rate (accepted / (accepted + rejected)) drops sharply, and do NOT declare the campaign "converged" or over just because the accept rate falls.** A run of rejections is normal in this regime and is still useful signal (it maps the wall). Keep probing: bit-for-bit structural/setup trims, creative math-preserving reformulations, and stacking several sub-5% bit-for-bit wins into one logged iteration (the "stack to 5%" ruling). Record every attempt (including rejects) in the history as usual.

## ⭐ PHASE 2 (CURRENT TARGET, 2026-06-03) — optimize `benchmark()`, NOT `compute_parameters()`

**Phase 1 (`compute_parameters()`) is DONE and validated — champion frozen, stop iterating it.** Final 1000-user re-validation (2026-06-03): **×103.8 median portable** (≈ the 50-user ×104.47 anchor → generalizes) / **×198 with the AVX2 bonus**; throughput 7.4k → 763k → 1.45M reviews/s. Records: `result/history.jsonl`+`.md` (every Phase-1 iter) and the 1000-user `.jsonl`s `result/{iter0-1000u-baseline,champ-1000u,avx2-1000u}.jsonl`. Mechanistic conclusion: the windowed forward is throughput-bound on the FMA ports + accuracy-band-locked, so no portable bit-for-bit lever > ~2% remains there. The AVX2 ~1.89× is a CPU-specific BONUS (x86-only → unofficial; build with `RUSTFLAGS="-C target-cpu=native"`).

**New target: make `benchmark()` (in `benchmark.py`) faster.** It was the reference/anchor harness throughout Phase 1, so it was never tuned — it deliberately keeps the **O(N²) per-prefix** path (`batch_loss_and_grad_simd` / `loss_and_grad_range_simd` in analytic.rs) as the bit-for-bit correctness anchor, plus 5-fold TimeSeriesSplit + a Python forgetting curve. It DID inherit the shared-kernel wins (analytic gradient, f32x8 SIMD, minimax, host batches, hand-rolled Adam → already ~2.1× on its Rust-training portion vs iter0), but its own O(N²) structure + harness scaffolding is the untuned mutation surface.

- **Same protocol** (min-of-3, `.pyd`-swap, **median speed_ratio ≥ 1.05**, complexity gate constraint 13) and **same constraints** (≤2 threads/user, fixed dataset/epochs/seed, NEVER modify `evaluate()`, portable per constraint 7). You'll need to set up a measurement harness that times `benchmark()`'s Rust region (analogous to `profiling/measure.py`, which times `compute_parameters()`).
- **Track Phase-2 history in SEPARATE files: `result/history_benchmark.jsonl` + `result/history_benchmark.md`** (do NOT touch the Phase-1 `history.jsonl`/`.md`). Same per-entry fields; its own plot.
- **⚠ ONLY BIT-FOR-BIT changes are viable.** `benchmark.py`'s unweighted mean log loss (**0.313500**) is already only ~5e-5 under the band ceiling (0.31355), so ANY inexact/precision change busts it. The O(N) window is OUT (its per-timestep loss ≠ the O(N²) per-prefix loss → not bit-for-bit). Hunt **bit-for-bit** wins only: redundant recomputation in the O(N²) prefix recurrence, harness/setup scaffolding, dedup, one-time hoists — the same KIND as Phase-1's iter19 (build-once) / iter21 (dedup) / iter25 (skip-last + scan-merge).
- **Huge head start vs starting from 0** (Andrew's point): the Phase-1 memory (`fsrs-speed-campaign-status`, `banked-speed-and-band-assets`), this CLAUDE.md, `result/history.jsonl` (every Phase-1 attempt + ruling), and a fully-working optimized `compute_parameters()` are all templates — the shared kernels (analytic.rs `curve8`/`stab8`/`exp8`/`ln8`, training.rs Adam/penalty/batching) are already optimized and proven correct.
