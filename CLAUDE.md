...
**✅ STATUS (2026-06-04): DONE — PR #417 OPENED** (open-spaced-repetition/fsrs-rs#417), AWAITING maintainer review. 3 bit-for-bit scalar commits (hoist + active-branch + padding-skip) → ~−22% windowed `compute_parameters`, log loss unchanged, rebased onto current upstream (incl #416), mergeable. Two more wins FLAGGED to the maintainers (dead-loss skip ~7% in the PR body; f32-backward ~5% in a PR comment) — each needs a test-tolerance tweak, their call. **SIMD/FMV PARKED** (auto-vectorized minimax kernel came out ~neutral on FSRS-6's already-fast PR#411 analytic gradient + can't validate multi-machine). Full detail + the f32-backward experiment (local branch `f32-backward-experiment`) in memory `fsrs-phase3-upstream-port`. NEXT: respond to maintainer review; if they OK relaxing the `windowed_matches_sum_of_prefixes` 1e-9 tolerance / the loss-assert, send the dead-loss + f32-backward follow-up. The directive that produced this:

**Phases 1 & 2 are DONE in THIS fork (FSRS-7), validated + pushed.** Phase 1 (`compute_parameters()`): **×103.8** 1000-user portable / ×198 AVX2 (champion frozen). Phase 2 (`benchmark()`): **×1.23** (iter1 +16% + iter2 +6%, both bit-for-bit; champion `.pyd` 3d1147ed; history in `result/history_benchmark.{jsonl,md}`). The fsrs-rs maintainers **MERGED a subset upstream in PR #411** — analytic gradient + host Adam + `card_ids` window, **scalar**, crediting this repo → ~9.3× on their `compute_parameters`. The SIMD did **not** replicate on Apple Silicon (it's x86-effective; ARM gets the algorithmic core ≈ ×32, see README portability section).
